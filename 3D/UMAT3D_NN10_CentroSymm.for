...
!C  UMAT FOR TESTING A ONE LAYER ORTHOTROPIC HSC-NN WITH ACTIVATIONS OF DEGREE 10 (TEN).
...
REAL(PREC),PARAMETER::SQ2=DSQRT(2.0D0),SQ6=DSQRT(6.0D0),SQ3=DSQRT(3.0D0),SQ23=DSQRT(2.0D0/3.0D0)
END MODULE DEFS
...
!    REAL(PREC),PARAMETER::TOL1=1.0E-006, TOL2=1.0E-009
	REAL(PREC),PARAMETER::TOL1=dble(1.0E-007)
...
!	END DO
	...
	DO JJ=1,NNODES,1
		...
		SG8(4)=SG2(4)*SG2(4)
		SG8(4)=SG8(4)*SG8(4)
		YG=YG+SG8(1)*SG2(1)+SG8(2)*SG2(2)+SG8(3)*SG2(3)+SG8(4)*SG2(4)
    END DO

!C	 CALCULATE YIELD FUNCTION VALUE
	YF=NORM*((0.25D0*YG)**0.1D0)
...
!CC      INTERNAL PARAMETERS
	REAL(PREC),DIMENSION(5)::SDEV, SW
	REAL(PREC)::YG, TT
	REAL(PREC),DIMENSION(4)::SG, SG2, SG8, WT
    INTEGER::JJ
!	REAL(PREC),DIMENSION(NTENS)::CSIGMA
    REAL(PREC),PARAMETER::NORM=100.0D0
...
!CC      INTERNAL PARAMETERS
    ...
    INTEGER::JJ
!	REAL(PREC),DIMENSION(NTENS)::CSIGMA
    REAL(PREC),PARAMETER::NORM=100.0D0,htol=dble(1.0e-12)
    REAL(PREC),PARAMETER::NEXP=10.0D0,NEXPONE=9.0D0
!C                       	
	REAL(PREC),DIMENSION(5)::DG
...
!	END DO
	...
	DO JJ=1,NNODES,1
		...
		DG(4)=DG(4)+VVWW(4,JJ)*WT(3)
		DG(5)=DG(5)+VVWW(5,JJ)*WT(4)
		WT(1)=SG8(1)+SG8(2)+SG8(3)+SG8(4)
		WT(2)=SG8(1)-SG8(2)-SG8(3)+SG8(4)
		WT(3)=SG8(1)-SG8(2)+SG8(3)-SG8(4)
		WT(4)=SG8(1)+SG8(2)-SG8(3)-SG8(4)
...
!C  NOTE: 4*0.5 = 2.0   ()	
	...
	GYF(5)=TT*DG(4)
	GYF(6)=TT*DG(5)
	HYF(1,1)=htol+NEXPONE*(TT*HGG(1)-(GYF(1)*GYF(1))/YF)/NORM
	...
	HYF(1,2)=HYF(2,1)
	HYF(2,2)=htol+NEXPONE*(TT*HGG(6)-(GYF(2)*GYF(2))/YF)/NORM
	...
	HYF(2,4)=HYF(4,2)
	HYF(3,4)=HYF(4,3)
	HYF(4,4)=htol+NEXPONE*(TT*HGG(10)-(GYF(4)*GYF(4))/YF)/NORM
	...
	HYF(3,5)=HYF(5,3)
	HYF(4,5)=HYF(5,4)
	HYF(5,5)=htol+NEXPONE*(TT*HGG(13)-(GYF(5)*GYF(5))/YF)/NORM
	...
	HYF(4,6)=HYF(6,4)
	HYF(5,6)=HYF(6,5)
	HYF(6,6)=htol+NEXPONE*(TT*HGG(15)-(GYF(6)*GYF(6))/YF)/NORM
	YF=NORM*YF	
